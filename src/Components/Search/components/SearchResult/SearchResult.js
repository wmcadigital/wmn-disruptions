...
class SearchResult extends React.Component {
  ...
  ToggleFav() {
    const { props } = this;
    const { AddFavourite, DeleteFavourite, isFav, favourites, searchResult } = props || {};
  ...
  SelectResult(e) {
    const { props } = this;
    const { ChooseResult, isFav, searchResult } = props || {};
    const { service, area, provider, route, disruption, mode } = searchResult || {};
  ...
  render() {
    const { props, ToggleFav, SelectResult } = this;
    const { searchResult } = props || {};

    return <SearchResultView toggleFav={ToggleFav} selectResult={SelectResult} searchResult={searchResult} />;
  }
}
...
const mapDispatchToProps = dispatch => {
  const { AddFavourite, DeleteFavourite, ChooseResult } = a || {};
  return {
